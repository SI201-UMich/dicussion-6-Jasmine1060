...
class PollReader():
    ...
    def build_data_dict(self):
        ...
        for i in self.raw_data[1:]:
            ...
            seperated = i.split(',')
            # when you do split, it turns into a string 
    ...
    def highest_polling_candidate(self):
        ...
        """
        #this sees which candidate has the highest polling percentage
        max_harris = max(self.data_dict['Harris result'])
        max_trump = max(self.data_dict['Trump result'])

        if max_harris > max_trump:
            return f"Harris {(max_harris * 100):.1f}" #the 1f is converting to two decimal places
...
#counts which samples are likely, and then it counts it
    ...
    def polling_history_change(self):
        ...
        """
        pass
        early_harris_sum = sum(self.data_dict['Harris result'][-30:])/30
        early_trump_sum = sum(self.data_dict['Trump result'][-30:])/30
        late_harris_sum = sum(self.data_dict['Harris result'][:30])/30
        late_trump_sum = sum(self.data_dict['Trump result'][:30])/30

        return (late_harris_sum - early_harris_sum, late_trump_sum - early_trump_sum)
